...
0级DOM <!-- 是 --> 一开始就有
0级DOM <!-- 是 --> onclick="alert('thanks');
0级DOM <!-- 是 --> document.getElementById("myButton").onclick = function () { alert('thanks'); }

1级DOM <!-- 没有 --> 定义事件相关的内容 1998年

2级DOM <!-- 分 --> 三个阶段
2级DOM <!-- 有 -->  1：事件捕获， 2：处于目标阶段， 3：事件冒泡阶段
2级DOM <!-- 是 --> addEventListener()
 
0级DOM事件模型处理<!-- 没有 -->capturing阶段

所有浏览器都<!-- 支持 -->event对象，但支持方式不同
IE的事件流<!-- 是 -->冒泡, 

event对象 的 stopPropagation()方法 <!-- 可以 --> 阻止事件传播进一步传播

2级DOM 的 第三个参数 是false <!-- 表示 --> 事件监听函数不能在 事件捕获(capturing阶段) 捕捉到事件

3级DOM <!-- 是 --> 增加了XPath模块 加载与保存（Load and Save）模块。
...
在传统语言里 构造函数 <!-- 是 --> 类中的一些特殊方法
在JavaScript里 构造函数 <!-- 只是 --> 一些使用 new 操作符时被调用的函数。
